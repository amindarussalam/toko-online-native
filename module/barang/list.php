...
<?php 
  ...
  $queryBarang = mysqli_query($koneksi, "SELECT barang.*, kategori.kategori FROM barang JOIN kategori ON barang.kategori_id=kategori.kategori_id $where ORDER BY barang.barang_id DESC LIMIT $mulai_dari, $data_perhalaman");
  ...
  if(mysqli_num_rows($queryBarang)==0){
    ...
    pagination($queryHitungBarang, $data_perhalaman, $pagination, "index.php?page=my_profile&module=barang&action=list$search_url");
